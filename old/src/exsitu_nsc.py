...
from NSC.old.src.help import check_finite_positive
...
def filter_tree_for_formation(full: TreeTable, final_redshift: float, mpb_only: bool) -> TreeTable:
    ...
    if mpb_only:
        keep = keep & (full.branch_id == full.mpb_branch_id)
...
def build_branch_release_map(full: TreeTable, final_redshift: float) -> dict[int, ReleaseInfo]:
    ...
    mpb_times = np.asarray([smhm.cosmicTime(z, units="Gyr") for z in full.redshift[mpb_indices]], dtype=float)
    ...
    for branch_id in sorted(set(int(v) for v in full.branch_id)):
        ...
        t_release_gyr = float(smhm.cosmicTime(z_release, units="Gyr"))
        ...
        r_release_kpc = (
            ...
            * check_finite_positive(smhm.virialRadius(mpb_mass_msun, z_release), "MPB virial radius at release")
            / 1.0e3
        )
...
def estimate_satellite_df_time_gyr(
    ...
    v_sat_kms = 0.977792221 * check_finite_positive(smhm.vvir(sat_mass, z), "satellite virial velocity")
