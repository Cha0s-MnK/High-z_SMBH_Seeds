...
The discovery of massive black holes in the first billion years of cosmic history provides a direct constraint on the formation of black-hole seeds and their early assembly. Luminous quasars at $z \gtrsim 6$ host black holes with masses of $M_{\bullet} \sim 10^8$--$10^{10}~M_\odot$, requiring either massive initial seeds, sustained near-Eddington accretion, episodes of super-Eddington growth, low radiative efficiencies, or a combination of these processes \citep[e.g.,][]{Fan+2023, Inayoshi+2020, Volonteri+2021}. The tension has become sharper with JWST, which has revealed a population of fainter active galactic nuclei at high redshift, including systems whose inferred black-hole masses appear large compared with the stellar masses of their young host galaxies \citep[e.g.,][]{Maiolino+2024GNz11, Natarajan+2024UHZ1, Furtak+2024A2744QSO1}. These objects extend the early-black-hole problem from the most luminous quasars to a broader population of lower-mass galaxies, where the ratio $M_{\bullet}/M_\star$ may be even more informative than $M_{\bullet}$ alone.
...
\czkong{[The next 3 paragraphs puts too many words on describing the model details, which should be investigated later in methods part. So Compress them into one paragraph with proper length.]}
...
\end{align}
where $u$ is the assigned enclosed mass fraction of a cluster within the formation event. Clusters that form on non-main branches are assigned local S{\'e}rsic radii within their own satellite branch when the ex-situ NSC mode is enabled. In the default accreted-cluster mode, they are instead placed at $0.5 R_{\rm vir}$ of the descendant halo rather than in the central S{\'e}rsic component.
...
At the end of each evolution segment, the solver writes one final state for every input GC. The status flag distinguishes six mutually exclusive fates. Clusters with status $1$ remain bound outside the nuclear sink and contribute their surviving stellar mass to the final-GC catalogue. Status $-1$ denotes complete stellar exhaustion outside the sink, after which no bound stellar component remains. Status $-2$ denotes direct tidal tearing outside the sink; the remaining stellar mass is deposited into the local radial bins and does not enter the central NSC aperture. Status $-3$ denotes an intact GC reaching $r_{\rm sink}=6\,{\rm pc}$ before disruption. In this case the remaining stellar mass is added to the cumulative NSC mass, $M_{\rm NSC}$, and any embedded IMBH is added to the cumulative central black-hole mass, $M_{\rm BH}$.

For IMBH-hosting clusters, loss of the stellar envelope converts the object into a naked wandering IMBH. Such remnants have status $-4$ if they remain outside the nuclear aperture at the final redshift, and status $-5$ if they later sink into the aperture. The latter contributes black-hole mass to $M_{\rm BH}$, but no stellar mass to $M_{\rm NSC}$. The deposited-mass profile therefore records stellar material lost outside the aperture, while the central NSC history records only mass carried into $r \le r_{\rm sink}$ by intact clusters plus IMBHs that cross the same sink boundary.

\subsection{Ex-situ NSC Mergers} \label{subsect:merge}

\czkong{[The latest version of the codes add an input param \texttt{--ex-situNSC 1} to add an ex-situ NSC merger channel. Describe the addtion here in this subsection.]}

The command-line flag \texttt{--ex-situNSC 1} activates a branch-continuation treatment for clusters formed outside the main progenitor branch. In this mode, a GC born on a satellite branch is first assigned a local S{\'e}rsic radius inside that satellite and is evolved with the same GC evolution solver using the satellite branch's own halo history. The branch is followed until its release redshift, defined by the fixed-tree descendant connection, or by the terminal branch redshift when the direct descendant row is absent. Any intact GC or wandering IMBH that crosses $r_{\rm sink}$ before release contributes to the satellite's cumulative $M_{\rm NSC}$ and $M_{\rm BH}$.

At the branch merger, these cumulative central masses are imported into the recipient branch as a compact satellite-NSC contribution. Surviving non-central GCs are released into the recipient halo at $0.5R_{\rm vir}$ and continue evolving there. The procedure is recursive, so central masses assembled in lower-level satellites can pass through intermediate branches before reaching the main progenitor. In the default \texttt{--ex-situNSC 0} mode this preprocessing is skipped, and non-main-branch clusters are treated only as ordinary ex-situ GCs in the recipient halo. Comparing the two modes therefore isolates the additional contribution from compact satellite nuclei.
...
This appendix reviews the cosmological equations used by the semi-analytic model to convert between redshift, cosmic time, halo virial quantities, and the analytic background potential. \czkong{[Not accurate. Your beginning should reflect the appendix title, like ``this appendix reviews the cosmology equations used in the semi-analytical model.'']} Throughout these calculations we adopt a standard spatially flat $\Lambda$CDM cosmology and neglect radiation, which is subdominant for the halo-evolution redshifts considered here. \czkong{[This expression is strange and uncommon in astronomy, using more precise way, like a standard $\Lambda$CDM cosmology but without radiation since the redshift range we considered is ...]} The expansion function is
\begin{align}
\Omega_{\Lambda ,0} & = 1 - \Omega_{\rm m, 0} ,\\
E(z) & = \dfrac{H (z)}{H_0} = \sqrt{\Omega_{\rm m, 0} (1 + z)^3 + \Omega_{\Lambda, 0}} .
\end{align}
\czkong{[What's the meaning of $E(z)$? You should explain every variable mentioned in the equations to avoid vague meanings.]} Here $E(z)$ is the dimensionless expansion function, $H(z)/H_0$; $H_0$ is the Hubble constant; $H(z)$ is the Hubble parameter at redshift $z$; $\Omega_{\rm m,0}$ is the present-day matter density parameter; and $\Omega_{\Lambda,0}$ is the present-day dark-energy density parameter. The current implementation uses a single cosmology for both the formation catalogues and the analytic orbital-evolution background. \czkong{[Illustris-based formation quantities are no longer used.]} We adopt $\Omega_{\rm m,0}=0.307$, $\Omega_{\Lambda,0}=0.693$, and $H_0=67.97\,{\rm km\,s^{-1}\,Mpc^{-1}}$, matching the DESI 2024 BAO plus CMB flat-$\Lambda$CDM constraints used in the code. \czkong{[Mention here that this is based on latest DSEI+BAO measurements.]}
