...
\czkong{[Write a brief summary of the subsections here. Mention the main content of each subsection explictly.]}

We now compare the fiducial $N_{\rm S}=2$ output of the $M_{\rm h}(z=0)=10^{12}$--$10^{14}~M_\odot$ Illustris-1-Dark halo sample with observational diagnostics of GCs, NSCs, and massive BHs. We first verify that the GC population reproduces the local GC-system mass--halo mass relation and the broad GC age--metallicity structure. We then test whether the ex-situ deposited component, measured as the evolved stellar mass inside $R_{\rm NSC}=6~{\rm pc}$, follows the observed $M_{\rm NSC}$--$M_\star$ scaling. The next two subsections connect this deposited stellar component to the central BH mass, $M_\bullet$, which is built from IMBH delivery into the fixed $1~{\rm pc}$ sink plus the central-only accretion prescription. We then present a local $z=0$ number-density diagnostic for the Nuclear and Satellite BH components. Finally, we use the QSO1 rotation-curve comparison as a case study of whether the fiducial catalogue produces a compact, BH-dominated nucleus similar to the observed source.

\subsection{Properties of GCs at $z = 0$ \& Comparison with Observations} \label{subsect:check}

\czkong{[Based on \texttt{\_plots\_Choksi+2018/Fig.03\_mgc\_vs\_mhalo.pdf} and \texttt{\_plots\_Choksi+2018/Fig.09\_age\_metallicity}. Refer to similar sections in Gao+2024 paper.]}
...
\czkong{[Based on \texttt{\_plots\_Neumayer+2020/Fig.12\_nsc\_scaling}. Refer to similar sections in Neumayer+2020 paper.}
...
\czkong{[Based on \texttt{\_plots\_Kong+2026/Fig.05\_Mbh$\sim$Mnsc.pdf}. Refer to similar sections in Neumayer+2020 paper.]}
...
\czkong{[Based on \texttt{\_plots\_Kong+2026/Fig.04\_Mbh$\sim$Mstar.pdf}. Refer to similar sections in Kritos+2025 paper.]}

We next compare the central BH mass with the stellar mass inferred for the same redshift. The stellar mass in this diagnostic is not measured from simulated star particles; it is obtained by applying the project stellar-mass--halo-mass helper to the main-progenitor halo mass at each output redshift. The comparison is therefore a consistency check between the cluster-seeded central BH channel and an empirical host-galaxy mass scale.

The fiducial tracks occupy the low-mass extrapolation of the local \citet{ReinesVolonteri2015} relation at early times and then move upward as central BH accretion proceeds. At fixed redshift, the model produces a broad range of $M_\bullet/M_\star$, with some high-redshift systems lying above the local relation for their inferred stellar mass. However, the most extreme observed JWST/LRD comparison points remain above the fiducial model at $z\sim7$: in the catalogue, the maximum central BH mass at $z=7$ is only $6.8\times10^5~M_\odot$. Thus, the $2$ per cent Eddington central-only accretion run can populate moderately overmassive early BHs, but it does not reproduce the full mass scale of the most extreme high-redshift AGN points without additional growth or a different seeding/accretion history.

\subsection{Massive BH Mass Function / Number Density} \label{subsect:BHMF}

\czkong{[Refer to similar sections in Kritos+2025 paper.]}

For the local abundance diagnostic, we follow the component split used in the Kritos+2025-style comparison. The Nuclear component consists of the positive central BH masses in the $z=0$ halo-summary rows, while the Satellite component consists of positive IMBHs that remain outside the nucleus, either in surviving GCs with status $1$ or as wandering IMBHs with status $-4$. Objects with status $-3$ or $-5$ have already entered the central sink and are therefore included only through the Nuclear channel. No Ejected component is included in this catalogue.

The resulting $n_{\rm BH}$ values should be interpreted carefully. They are selected-output Kong counts divided by the Illustris-1-Dark box volume, $V=1.20911\times10^6~{\rm Mpc}^{3}$, following the adopted Fig.09 normalisation; they are not a parent-halo-selection-weighted reconstruction of the full Illustris-1-Dark halo population. With this convention, the $z=0$ inventory contains 128 Nuclear BHs with masses $3.4\times10^7$--$1.6\times10^9~M_\odot$ and $8.4\times10^5$ Satellite IMBHs with masses $1.0\times10^2$--$3.0\times10^3~M_\odot$. The Satellite component dominates the number density at IMBH masses, peaking near the $8.5\times10^2$--$1.7\times10^3~M_\odot$ bin, while the Nuclear component is restricted to the massive-BH regime and peaks at $n_{\rm BH}\simeq2.6\times10^{-5}~{\rm Mpc}^{-3}$ near $10^8~M_\odot$. Under this selected-output normalisation, the Nuclear curve remains below the Greene et al.\ lower-limit curves used in the Kritos-style reference comparison, so this panel is best viewed as an abundance diagnostic for the simulated sample rather than as a complete local BH mass function.

\subsection{Case Study: Is Abell 2744-QSO1 a ``naked'' SMBH?} \label{subsect:case}

\czkong{[Based on \texttt{\_plots\_Kong+2026/Fig.08\_QSO1\_rotation\_curve\_mass\_profile}. The Juodzbalis+2026 paper gives several mutually reinforcing reasons that QSO1 contains an accreting central compact object. The defensible reinterpretation is narrower: QSO1 may be a SMBH embedded in a compact nuclear star cluster assembled by GC inspiral and IMBH delivery, rather than a completely naked point-mass BH in an otherwise empty nucleus. Use the given figure to write to support this idea.]}

The QSO1 comparison is a useful stress test because it asks whether the fiducial catalogue can produce a compact, BH-dominated nucleus at $z\simeq7$. We compare the observed rotation-curve points and point-mass curve with the deposited-stellar-plus-central-BH velocity profiles of the model haloes at $z=7$. The best-matching profile in the existing catalogue is halo \texttt{1660}. Its line-of-sight velocity profile reaches $63.8~{\rm km~s^{-1}}$ at $12.5~{\rm pc}$, $32.2~{\rm km~s^{-1}}$ at $50~{\rm pc}$, and $18.6~{\rm km~s^{-1}}$ at $150~{\rm pc}$, giving a reasonable match to the shape of the plotted rotation curve.

The mass decomposition, however, does not support the intended SMBH-in-a-dynamically-subdominant-NSC interpretation for this fiducial run. The same best-matching halo has a central BH mass of only $1.2\times10^4~M_\odot$ at $z=7$, whereas the deposited stellar mass enclosed within $150~{\rm pc}$ is $1.2\times10^7~M_\odot$. The stellar component contributes more than $99$ per cent of the enclosed mass over the radii used for the comparison. Thus, the existing catalogue evidence indicates that the QSO1-like velocity amplitude in this run is produced by a compact deposited stellar nucleus rather than by an SMBH embedded in a dynamically subdominant NSC\@. This negative result is informative: the fiducial GC-inspiral model can build compact nuclear stellar mass early, but matching QSO1 as a BH-dominated object requires substantially more central BH growth than is present in the $Eddington=0.02$ catalogue.
...
The production runs use the Chen--Gnedin mass--metallicity relation \citep{Chen&Gnedin2024aGC},
\begin{align}
