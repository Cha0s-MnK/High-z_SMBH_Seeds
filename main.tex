...
\includegraphics[width=\linewidth]{fig/flow_chart.png}
\caption{Flow chart.}
\label{fig:flow_chart}
\end{figure*}

\czkong{Too many equations in the method part. Only keep important ones here and move others into the appendix.}
...
\end{align}
where $u$ is the assigned enclosed mass fraction of a cluster within the formation event. Clusters that form on satellite branches are assigned local S{\'e}rsic radii within their own branch before any later halo merger. If they survive outside the nuclear region until the branch merges, they are released into the recipient halo at $0.5R_{\rm vir}$ and continue evolving in the new host potential.
...
\end{align}
with $\sigma_m = 0.3$ dex. This scattered cluster metallicity is used both for stellar-evolution bookkeeping and for the IMBH seed calculation. For the IMBH module, the cluster abundance is converted to a solar-scaled metallicity through
\begin{align}
...
For continuous tidal stripping we follow the local-orbit prescription of \cite{Fragione+2019GC}. The stripping rate is
\begin{align}
\dfrac{\mathrm{d} M_{\rm strip}}{\mathrm{d} t} & = \dfrac{2^{2 / 3} \left( \dfrac{M}{10^5~M_\odot} \right)^{1 / 3}}{10 P(r)} , \\
...
\end{align}
where $M$ is the current inspiralling mass. The code integrates this radial decay with a fourth-order Runge--Kutta estimate that re-evaluates the enclosed deposited mass and analytic background density at substeps. Objects reaching $r_{\rm sink} = 10^{-3}\,{\rm kpc}$ enter the fixed nuclear sink. Their stellar component is deposited into the innermost radial bin, while any embedded or wandering IMBH is added to the central black-hole channel.
...
\czkong{Add description here to show the current modelling of Eddington-limited nuclear BH growth.}
...
The merger treatment follows satellite branches explicitly before their material is handed to the descendant branch. A GC born on a satellite branch is assigned a local S{\'e}rsic radius inside that satellite, using the spatial-sampling procedure in Sect.~\ref{subsubsect:spatial_sampling} and equation~(\ref{eq:sersic_density}). It is then evolved with the same stellar-evolution, tidal-stripping, tidal-tearing, and dynamical-friction solver described in Sect.~\ref{subsect:gc_evolution} and equations~(\ref{eq:stellar_evolution})--(\ref{eq:dynamical_friction}), but using the satellite branch's own halo history. The branch is followed until the fixed-tree descendant connection releases it into its recipient branch, or until the terminal branch redshift when no direct descendant row is present.
...
We now compare the fiducial $N_{\rm S}=2$ output of the $M_{\rm h} (z = 0) = 10^{11}$-$10^{14}~M_\odot$ Illustris-1-Dark halo sample with observational diagnostics of GCs, NSCs, and massive BHs. We first verify that the GC population reproduces the local GC mass-halo mass relation and the broad GC age-metallicity structure. We then test whether the ex-situ deposited component, measured as the evolved stellar mass inside $R_{\rm NSC} = 6~{\rm pc}$ \citep{Neumayer+2020NSC}, follows the observed $M_{\rm NSC}$-$M_\star$ scaling. The next two subsections connect this deposited stellar component to the central BH mass, $M_\bullet$, which is built from IMBH delivery into the fixed $1~{\rm pc}$ sink plus the central-only accretion prescription. We then present a redshift-resolved BH mass-function diagnostic for the nuclear and satellite BH components. Finally, we use the QSO1 rotation-curve and UV comparisons to test whether the fiducial catalogue produces a compact nucleus containing a central BH and an NSC similar to the observed source.
...
The fiducial tracks occupy the low-mass extrapolation of the local \citet{ReinesVolonteri2015} relation at early times and then move upward as central BH accretion proceeds. At fixed redshift, the model produces a broad range of $M_\bullet/M_\star$, with some high-redshift systems lying above the local relation for their inferred stellar mass. \czkong{Change the following words from here. We discussed the most extreme case later and confirm that it can actually explained by our model using a NSC senario. So generally our model matches the observations well.} The highest-redshift JWST/LRD comparison points are among the most demanding cases, but the QSO1 case study below shows that their compact nuclear signal need not be interpreted as a naked BH: a GC-origin NSC containing a central BH can reproduce the relevant stellar and kinematic diagnostics. The model therefore reproduces the broad observed trend, while the detailed partition between BH and NSC remains an important uncertainty.

\subsection{Massive BH Mass Function / Number Density} \label{subsect:BHMF}
...
For the redshift-resolved BH mass function, \czkong{No longer ``local'' abundance now, we now plot redshift-based BHMF. Change corresponding descriptions here.} we follow the component split used in the Kritos+2025-style comparison. The Nuclear component consists of the positive central BH masses in the halo-summary rows at each output redshift, while the Satellite component consists of positive IMBHs in the final inventory that remain outside the nucleus, either in surviving GCs with status $1$ or as wandering IMBHs with status $-4$. Objects with status $-3$ or $-5$ have already entered the central sink and are therefore included through the Nuclear channel at the corresponding output redshifts. No Ejected component is included in this catalogue.

The resulting $n_{\rm BH}$ values should be interpreted carefully. The updated run uses a $16~{\rm cMpc}$ side-length cube selected to reproduce the halo mass function of the full Illustris-1-Dark volume, so we normalise the counts by $V=(16~{\rm cMpc})^3=4096~{\rm cMpc}^{3}$. \czkong{This has been changed since we now run the model on a 16 cMpc side length cube selected with halo mass function mimicing the whole Illustris-1-Dark box. Find the detailed codes in \texttt{/lingshan/disk3/subonan/Illustris-1-Dark\_Cube}.} The redshift-resolved Nuclear component contains 334 positive entries at $z=0$ and 7 at $z=10$, while the final Satellite inventory contains 19,144 positive IMBH entries with masses $1.0\times10^2$--$2.7\times10^3~M_\odot$. The Satellite component therefore dominates the number density in the IMBH-mass regime. The Nuclear and Satellite curves do not represent the same epoch: the former is evaluated at each output redshift, whereas the latter is a final-inventory measurement. Under this cube normalisation and epoch convention, Fig.~\ref{fig:BHMF} is an abundance diagnostic for the simulated sample rather than a complete cosmological BH mass function. \czkong{Now the output directory has been changed to \texttt{/lingshan/disk3/subonan/\_outputs/Cube\_Eddington0.3\_Mc7}. Change the data here correspondingly.}
...
\caption{RotationCurve.}
\label{fig:RotationCurve}
\end{figure}

The QSO1 comparison is a useful stress test because it asks whether the fiducial catalogue can produce a compact nuclear system at $z\simeq7$. We compare the observed rotation-curve points and point-mass curve with the deposited-stellar-plus-central-BH velocity profiles of the model haloes at $z=7$. The joint Keplerian+UV selection identifies halo \texttt{7967}. Its line-of-sight velocity profile reaches $52.7~{\rm km~s^{-1}}$ at $12.5~{\rm pc}$, $26.5~{\rm km~s^{-1}}$ at $50~{\rm pc}$, and $15.3~{\rm km~s^{-1}}$ at $150~{\rm pc}$, providing a compact velocity profile comparable to the plotted QSO1 data. \czkong{Now the data has changed. Change the words corrrepondingly.}

The selected halo contains an evolved NSC mass of $7.4\times10^6~M_\odot$ inside $6~{\rm pc}$ and a central BH mass of $1.7\times10^4~M_\odot$ at $z=7$. Over the $12.5$--$150~{\rm pc}$ range used for the rotation curve, the deposited stellar mass is $8.0$--$8.2\times10^6~M_\odot$, so the central BH contributes less than $0.3$ per cent of the enclosed mass. The BH is included in the velocity calculation, but the predicted amplitude is therefore dominated by the compact stellar component. This result is consistent with a nuclear star cluster assembled by GC inspiral, with IMBH delivery supplying a central BH within the cluster. It supports an embedded-BH interpretation of the compact QSO1 signal, although this particular fiducial halo is not a BH-dominated dynamical match to the observed source.

\begin{figure}[htbp!] \centering
\includegraphics[width=\linewidth]{fig/Fig.05_UVmag.pdf}
\caption{UV magnitude.}
\label{fig:RotationCurve}
\end{figure}

\czkong{Add description for this new figure Fig.05\_UVmag. Use this to support the ``SMBH embedded in a compact
nuclear star cluster assembled by GC inspiral and IMBH
delivery'' idea.}

The UV-aperture diagnostic provides an independent check on the compact stellar interpretation. For the selected halo, the GC-origin stellar component has a rest-frame $1500~\AA$ absolute AB magnitude of $M_{\rm UV}\simeq-14.52$--$-14.54$ as the aperture increases from $1$ to $7~{\rm pc}$. This value is nearly independent of aperture over the plotted range and is about $1.1$ mag fainter than the QSO1 reference value, $M_{\rm UV}=-15.60$. The calculation includes the stellar light from the deposited GC population only, so it shows that a compact NSC can provide a substantial fraction of the observed UV emission; any remaining flux may be supplied by an active central BH. Together with the rotation curve, this supports an interpretation in which a central BH is embedded in a compact NSC assembled by GC inspiral and IMBH delivery, while the UV comparison alone does not determine the relative AGN contribution.
...
\czkong{Add description for this new figure Fig.04\_BHmasses. Final summary the estimated mass for the ``SMBH embedded in a compact nuclear star cluster assembled by GC inspiral and IMBH delivery'' case.}

For the same joint Keplerian+UV-selected halo, Fig.~04 gives $M_{\rm NSC}(<6~{\rm pc})=7.4\times10^6~M_\odot$ ($\log_{10} M_{\rm NSC}/M_\odot=6.87$) and $M_\bullet=1.7\times10^4~M_\odot$ ($\log_{10} M_\bullet/M_\odot=4.23$) at $z=7$. The NSC is therefore about $4.4\times10^2$ times more massive than the central BH in this model case. The $6~{\rm pc}$ pure-stellar UV estimate used in the joint selection is $M_{\rm UV}=-14.62$, close to the value shown in the aperture diagnostic. These masses summarise a compact GC-delivered nuclear component containing a central BH, rather than a naked BH; the model point should not be read as reproducing the much larger BH masses inferred from the QSO1 dynamical estimates. The comparison instead demonstrates that the observed compact signal can be compatible with an NSC assembled by GC inspiral and a central BH grown from delivered IMBHs.
...
{t_i-t_{\rm prog}} > p_3 .
\end{align}
Here $t_i$ and $t_{\rm prog}$ are the cosmic ages of the current halo and its progenitor, and $p_3$ is the threshold parameter.
...
\section{Intermediate-Mass Black Hole Seeding} \label{append:IMBH}

The cluster half-mass radius used for IMBH seeding is
\begin{align}
r_{\rm h} & = \dfrac{f_{\rm h} R_4}{1.3} \left( \dfrac{M_{\rm cl}}{10^4~M_\odot} \right)^\beta ,
\end{align}
with $f_{\rm h} = 0.125$, $R_4 = 2.365~{\rm pc}$, and $\beta = 0.18$. For a Plummer profile, the projected half-mass radius is
\begin{align}
