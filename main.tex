...
\centering
\small
\caption{Final GC and IMBH status flags.}
\label{tab:final_status}

\begin{tabular}{cl}
\hline
\textbf{Status} & \textbf{Fate and bookkeeping} \\
\hline

$1$ &
\parbox[t]{0.84\textwidth}{\textbf{Surviving bound GC.}
A bound stellar cluster remains outside the fixed $1~\mathrm{pc}$ sink and
contributes its surviving mass to the final-GC catalogue. Earlier stripped
mass remains in the deposited-mass profile.} \\[6pt]

$-1$ &
\parbox[t]{0.84\textwidth}{\textbf{Stellar exhaustion.}
The bound stellar component is completely removed outside the sink. No
surviving GC or central black-hole entry remains, but the lost stellar mass
remains in the deposited-mass profile.} \\[6pt]

$-2$ &
\parbox[t]{0.84\textwidth}{\textbf{Direct tidal tearing.}
A non-IMBH-hosting cluster is tidally destroyed outside the sink. Its
remaining stellar mass is deposited at the disruption radius.} \\[6pt]

$-3$ &
\parbox[t]{0.84\textwidth}{\textbf{Sunk bound GC.}
A bound GC reaches the fixed $1~\mathrm{pc}$ sink before disruption. Its
remaining stellar mass is deposited in the innermost radial bin, and any
embedded IMBH enters the central black-hole channel.} \\[6pt]

$-4$ &
\parbox[t]{0.84\textwidth}{\textbf{Non-central wandering IMBH.}
An IMBH-hosting cluster loses its stellar envelope before reaching the sink.
The naked IMBH remains outside the nuclear region, while the stripped
stellar envelope is deposited where it was removed.} \\[6pt]

$-5$ &
\parbox[t]{0.84\textwidth}{\textbf{Sunk wandering IMBH.}
A naked wandering IMBH reaches the fixed $1~\mathrm{pc}$ sink and enters the
central black-hole channel. No stellar mass is deposited at entry.} \\
...
\textit{Note.}
The published stellar NSC mass, $M_{\rm NSC}$, is sampled from the cumulative
evolved stellar-mass deposition profile inside $R_{\rm NSC}=6~\mathrm{pc}$.
The fixed $1~\mathrm{pc}$ sink is the innermost deposit bin and the
central-BH entry boundary, not the full public NSC aperture.
}
...
\begin{figure}[htbp!] \centering
\includegraphics[width=\linewidth]{fig/Fig.03_BHseed_hist.pdf}
\caption{Cosmic formation history of intermediate-mass black-hole (IMBH) seeds. The upper panel shows the differential seed-formation rate, $\mathrm{d}n_{\rm seed}/\mathrm{d}\log_{10}(1+z)$, and the lower panel shows the cumulative seed density, $n_{\rm seed}(>z)$, as functions of $\log_{10}(1+z)$; the upper axis gives the corresponding redshift. The purple curve labelled ``This work'' includes every final-GC row with $M_{\rm IMBH,init}>0$, weighted to the full TNG50 reference volume; its rate bins have width $\Delta\log_{10}(1+z)=0.02$ and its cumulative curve uses the strict condition $z_{\rm form}>z$. The black, blue, red, and green curves show the All seeds, Pop-III (sub-Eddington), Pop-III (Eddington), and Pop-II histories from Chen+2026. \czkong{Add caption for this cosmic BH seeding history.}}
\label{fig:BHseed_hist}
\end{figure}

\czkong{Add a para here to describe the cosmic BH seeding history.}

Figure~\ref{fig:BHseed_hist} follows the formation of the IMBH seeds assigned at GC birth. In this catalogue, all 4,793,505 rows with positive $M_{\rm IMBH,init}$ lie within the plotted range $0\leq z_{\rm form}\leq50$; after the TNG50/TNG100 volume weighting, they correspond to an effective count of $1.024\times10^6$. The differential rate has a broad maximum at $z\simeq2.4$ with $\mathrm{d}n_{\rm seed}/\mathrm{d}\log_{10}(1+z)\simeq24.8~{\rm cMpc}^{-3}~{\rm dex}^{-1}$, while the cumulative density decreases from $7.42~{\rm cMpc}^{-3}$ near $z=0$ to $0.45~{\rm cMpc}^{-3}$ at $z\simeq7$ and $2.2\times10^{-2}~{\rm cMpc}^{-3}$ at $z\simeq10$. The purple history lies above the Chen+2026 comparison curves at lower redshift but turns over towards earlier times; because it is the volume-normalised event history of the final-GC catalogue rather than a continuous cosmic star-formation prediction, this comparison primarily illustrates the different seed-formation prescriptions and catalogue normalisations.
...
At $z=7$, halo 221 has $M_{\rm UV}=-15.46$ from its GC-origin stellar population over $1$--$7~{\rm pc}$, only $0.14$ mag fainter than the QSO1 reference, alongside $M_{\rm NSC}(<6~{\rm pc})=6.72\times10^6~M_\odot$ and $M_\bullet=2.12\times10^4~M_\odot$. The pure-stellar UV estimate excludes AGN, nebular, dust, and in-situ young-stellar emission, and the model masses are not intended to reproduce the much larger dynamical BH estimates; together with the rotation curve, they instead support a compact NSC containing a central BH grown from delivered IMBHs.
...
These limits can be compared with the minimum halo mass expected to host a GC in our model. According to \citet{Choksi&Gnedin2019GC}, the GC mass--halo mass ratio at \(z\sim10\) is approximately \(3\times10^{-4}\). A halo hosting a GC with our minimum cluster mass, \(M_{\rm min}=10^5~M_\odot\), therefore requires roughly
...
\begin{align}
\dfrac{\mathrm{d} \Phi_{\rm NFW}}{\mathrm{d} r} = \dfrac{G M_{\rm vir}}{\ln (1 + c) - c / (1 + c)} \left[ \dfrac{\ln (1 + r / R_s)}{r^2} - \dfrac{1}{r (r + R_s)} \right] ,
\end{align}
...
\end{align}
Events with $M_{\rm GC, tot}<10^5~M_\odot$ do not form a cluster in the current implementation. \czkong{Check this. This may have significant impact on the results.}
...
\label{eq:cimf}
\end{align}
where $A_{\rm c}$ is the normalisation and $M_{\rm c}$ is the exponential cut-off mass. We use $M_{\min}=10^5~M_\odot$; the fiducial runs use $M_{\rm c}=10^7~M_\odot$ as described in \cite{Choksi&Gnedin2019GC}. The upper incomplete gamma function is
\begin{align}
...
-\Gamma\left(-1,\dfrac{M_{\max}}{M_{\rm c}}\right)}.
\end{align}
Sampling continues until the event mass budget is exhausted. If the final draw exceeds the remaining budget, its mass is set to the remaining mass. \czkong{What if the remaining mass $< M_{\rm min} = 10^5~M_\odot$?}The cluster list is shuffled \czkong{More details here.} before radii are assigned, so the deterministically reserved maximum-mass cluster is not always placed at the smallest S{\'e}rsic radius.
...
For main-progenitor and locally sampled satellite clusters \czkong{This should apply to any cluster.}, the birth radius is drawn from the deprojected S{\'e}rsic profile used by \cite{Gao+2024GC},
\begin{align}
...
\end{align}
where $j_{\rm sp}$ is the specific angular-momentum norm of the halo, $h=H_0/(100~{\rm km/s/Mpc})$. The enclosed-mass fraction is inverted through the regularised incomplete gamma function,
\begin{align}
r_{\rm GC} & = R_{\rm e}
...
\end{align}
In the analytic background, the corresponding spin parameter and effective radius are \citep{Gao+2024GC} \czkong{Check this.}
\begin{align}
\lambda & = \dfrac{J}{\sqrt{2GR_{\rm vir}M_{\rm h}}},\\
...
The IMBH module assigns a seed once, at GC birth, using the runaway-collision prescription of \cite{Rantala+2026FROST&IMBH}. Dense, metal-poor clusters can build very massive stars through stellar collisions before those products collapse into IMBHs. The cluster structural scale is the three-dimensional half-mass radius
\begin{align}
r_{\rm h} & =
...
%The implementation evaluates the supplied positive metallicity ratio in its corresponding piecewise interval and issues a warning when it lies outside this range; it does not clip the input.
...
\begin{align}
\rho_{\rm bg} (r) &=
\dfrac{3}{4 \pi r^3} \left[ r^2 \dfrac{\mathrm{d} \Phi_{\rm NFW}}{\mathrm{d} r} + \dfrac{G M_\star (< r)}{r} \right] .
\end{align}
...
\begin{align}
\dfrac{\mathrm{d} r}{\mathrm{d} t} & = - \dfrac{1}{0.45 r v_{\rm c} (r)} \left( \dfrac{M_{\rm GC}}{10^5~M_\odot} \right) = - f (r),
\label{eq:DF}
\end{align}
...
\begin{align}
\dfrac{\mathrm{d} M_{\rm GC}}{\mathrm{d} t} & = \dfrac{2^{2/3} M_{\rm GC}^{1/3} v_{\rm c}}{r} .
...
\right]^{1/(\alpha+1)}
\end{align}
for the bounded interval $M_{\min}\leq M\leq M_{\max}$, where $r$ is a uniform deviate on $[0,1)$. \czkong{If only used in GC mass sampling, then move to there.}
...
e^{-(x-x_i)^2/(2h^2)},
\end{align}
where $h$ is the bandwidth, $x_i$ are the samples, $w_i$ are optional weights, and $N$ is the number of samples. \czkong{Was this used?}
...
Together, Figures~\ref{fig:UVmag} and~\ref{fig:masses} show that the same selected halo can match the compact stellar diagnostics without requiring a dynamically dominant central BH.
